...
void render_current_game_id(void)
{
    for (int i = 0; i != settings.nb_games; i++) {
        if (i+1 == infos.game_id) {
            display_rectangle(50 + 100 * i, infos.height - 75, 75, 40, color.dark_gray, 1, 5, color.cyan);
            display_text(int_to_str(i+1), 75 + 100 * i, infos.height - 80, sfWhite, 40, 1);
        } else {
            display_rectangle(50 + 100 * i, infos.height - 75, 75, 40, sfWhite, 1, 5, color.cyan);
            display_text(int_to_str(i+1), 75 + 100 * i, infos.height - 80, sfBlack, 40, 1);
        }
    }
}
